...
extension IPPacket: CustomStringConvertible {
    var description: String {
        "Version: \(version.rawValue), Length: \(length), TTL: \(timeToLive), Protocol: \(proto), Source: \(source):\(sourcePort), Dest: \(destination):\(destinationPort), "
    }
}
...
extension IPPacket {
    var tcpHeader: TCPHeader {
        let tcpHeaderOffset = Int(headerLength) + 13
        let flags = packetData[tcpHeaderOffset]
        return TCPHeader(flags: flags)
    }
    
    mutating func setTCPFlags(syn: Bool = false, ack: Bool = false, fin: Bool = false, rst: Bool = false) {
        let tcpHeaderOffset = Int(headerLength) + 13
        var flags: UInt8 = 0
